...
The AI proposes a destination, and after your go-ahead `student-wiki file` copies the file here as `raw/{COURSE}/{type-folder}/YYYY-MM-DD-{type}-{short-description}.{ext}`. The original location, original filename, and SHA-256 are recorded in `raw/.manifest.json`.
