...
The user gives the location of a file anywhere on disk. `student-wiki file` copies it into `raw/`, renames it, hashes it, and records where it came from. Files enter `raw/` only through this script.
...
2. **Propose**: Run the script with `--dry-run` and show the user `{original path} → {raw path}`. Wait for the go-ahead; a name in `raw/` is permanent
   ```
   student-wiki file "{path}" --course COMP6713 --type lecture [--date YYYY-MM-DD] [--slug text] --dry-run
   ```
...
6.2. **Extract assessments** (for `admin`, `assignment`, and `exam` sources): follow "Extract During Ingest" in the wiki-tracker skill. Propose the assessments found in the source as a table, copy dates and weights only as printed, wait for confirmation, then add them with `student-wiki tracker add --source {source-page}`
