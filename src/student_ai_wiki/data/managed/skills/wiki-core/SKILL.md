...
You maintain a student knowledge wiki. Read `raw/`, write `wiki/`. `raw/` is append-only: sources enter through `student-wiki file`, and a filed source is never edited, renamed, or deleted.
...
File: `wiki/tracker/{COURSE}-{slug}.md`, created and changed through `student-wiki tracker`. Fields: `type`, `course`, `title`, `due`, `due_time`, `weight`, `status`, `start_by`, `mark`, `out_of`, `hurdle`, `concepts`, `sources`, `needs_check`. The wiki-tracker skill holds the full format and the commands.

## Hot Cache Contents

After each operation, update `wiki/hot.md` (≤500 words): upcoming deadlines (the `## Upcoming` section, written by `student-wiki tracker hot`) / three most recent sources / recent concepts / current weak concepts / pending tasks
...
1. `raw/` is append-only: file sources with `student-wiki file`; never edit, rename, or delete a filed source
...
6. Tracker dates, priorities and grade figures come from `student-wiki tracker`; report its output as printed
7. `student-wiki` commands run from anywhere inside the vault. If the shell cannot find `student-wiki`, ask the student to run `pipx ensurepath` and open a new terminal, then continue
