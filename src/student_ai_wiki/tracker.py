...
Usage:
  student-wiki tracker add --type assignment --course COMP9417 --title "Assignment 2"
        --due 2026-10-12 --time 23:59 --weight 20 [--concepts A,B] [--source PAGE] [--dry-run]
  student-wiki tracker update ID [--due ...] [--status doing] [--clear FIELD]
  student-wiki tracker done ID
  student-wiki tracker mark ID 17 --out-of 20
  student-wiki tracker list [--course C] [--within DAYS] [--status S] [--type T] [--all]
  student-wiki tracker plan ID [--steps "Outline;Build;Submit"] [--start-by DATE]
  student-wiki tracker focus COURSE
  student-wiki tracker brief [--days 14] [--max 5] [--course C] [--json] [--hook]
  student-wiki tracker hot
  student-wiki tracker grades [--course C] [--target 75] [--what-if ID=85]
  student-wiki tracker target COURSE 75
  student-wiki tracker ics [--out calendar/student-wiki.ics] [--publish-gist]
  student-wiki tracker check

Every command takes --root DIR (default: STUDENT_WIKI_ROOT, then the vault containing the
...
from .file_source import slugify
from .vault import ROOT_HELP, resolve_root
...
class Tracker:
    def __init__(self, args):
        self.root = resolve_root(args.root)
...
def cmd_hot(tracker: Tracker, args) -> None:
    ...
    if len(rows) > HOT_MAX_LINES:
        rows = rows[:HOT_MAX_LINES - 1] + [f"- +{len(rows) - HOT_MAX_LINES + 1} more: run `student-wiki tracker brief`"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--root", help=ROOT_HELP)
    ...
    parser = argparse.ArgumentParser(prog="student-wiki tracker",
                                     description="Track assessments, deadlines, exams and to-dos.")
